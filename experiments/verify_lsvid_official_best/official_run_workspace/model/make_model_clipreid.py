...
class build_transformer(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        ...
        self.SAT = Transformer_SP(width=768, layers=1, heads=12, droppath=None, T=cfg.INPUT.SEQ_LEN)
    ...
    def forward(self, x = None, get_image = False, cam_label= None, view_label=None, text_features2=None):
        ...
        # if get_text == True:
        #     prompts = self.prompt_learner(label)  # torch.Size([64, 77, 512])
        #     ##########################
        #     B, L, D = prompts.shape
        #     prompts = prompts.view(-1, L, D)
        #     ##########################
        #     text_features = self.text_encoder(prompts, self.prompt_learner.tokenized_prompts)
        #     # text_features = self.text_encoder(prompts, self.prompt_learner.tokenized_prompts, self.prompt_learner.num_ctx)
        #     ##########################
        ...
        if self.model_name == 'RN50':
            ...
            f_sp = self.SAT(image_features_SAT)  # torch.Size([130, 64, 768])
        ...
        if self.training:
            ...
            text_features2 = text_features2 + self.prompts_generator(text_features2,
